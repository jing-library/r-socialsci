...
> [comparison](https://mgimond.github.io/rug_2019_12/Index.htm://mgimond.github.io/rug_2019_12/Index.html)
...
hence its name. If the column `no_listed_items` is true, the household does not 
own any listed items. We do not want to count the results from that column in. 
We then group the data by villages and calculate the mean number of items, so 
each average is grouped by village.

```{r, purl = FALSE}
interviews_items_owned %>%
    mutate(number_items = rowSums(select(., bicycle:car, -no_listed_items))) %>%
    group_by(village) %>%
